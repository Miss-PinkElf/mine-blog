...
## BUG-004 · prompt-2 日常/图生图 524 + curl exit 16（2026-07-17）

- **现象**：
  - 大设定图 `image_edit`（~1MB body）多次 HTTP 524
  - 文生图偶发 524 或 `curl exit=16`（HTTP/2 framing）
  - 单次失败约 70–130s 才返回，不是本地静默卡住
- **原因**：中转网关/上游超时与连接抖动；复杂构图更易 524
- **解决方案**：
  - 单次调用 `--retries 0` + `CURL_MAX_TIME=300`（避免长队列）
  - 优先 **文生图** 简化 prompt；失败后换更简单日常切片再试
  - 成功交付：daily-01/02/03 + stickers（model=gpt-image-2）
- **状态**：已绕过并交付；中转不稳仍可能复发
